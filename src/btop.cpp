    ...
    while (not force or (Term::width < minWidth or Term::height < minHeight)) {
        ...
        if (Term::width < minWidth or Term::height < minHeight) {
            ...
            cout << fmt::format("{clear}{bg_black}{fg_white}"
                                ...
                                "clear"_a = Term::clear, "bg_black"_a = Global::bg_black,
            ...
            if (got_key) {
                auto key = Input::get();
                // Logger::warning(fmt::format("Input read key: {}", key));
                if (key == "q")
                    clean_quit(0);
...
namespace Runner {
    ...
    static void *_runner(void *) {
        ...
        if (pt_lck.status != 0) {
            Global::exit_error_msg = "Exception in runner thread -> pthread_mutex_lock error id: " + to_string(
                                         pt_lck.status);
        ...
        while (not Global::quitting) {
            ...
            try {
                ...
                const bool gpu_in_cpu_panel = Gpu::gpu_names.size() > 0 and (
                                                  Config::getS("cpu_graph_lower").starts_with("gpu-") or Config::getS(
                                                      "cpu_graph_upper").starts_with("gpu-")
            ...
            if (output.empty() and not pause_output) {
                if (empty_bg.empty()) {
                    ...
                    empty_bg = fmt::format(
                        ...
                        "titleFg"_a = Theme::c("title"), "b"_a = Fx::b, "hiFg"_a = Theme::c("hi_fg"),
            ...
            cout << Term::sync_start << (conf.overlay.empty()
                                             ...
                                             : (output.empty()
                                                    ? ""
                                                    : Fx::ub + Theme::c("inactive_fg") + Fx::uncolor(output)) + conf.
                                               overlay)
                    << Term::sync_end << flush;
        }
        //* ----------------------------------------------- THREAD LOOP -----------------------------------------------
...
int main(const int argc, const char **argv) {
    ...
    Global::debug = cli.debug; {
        ...
        if (config_dir.has_value()) {
            ...
            if (not fs::exists(Theme::user_theme_dir, error) and not fs::create_directories(
                    Theme::user_theme_dir, error)) {
    ...
    if (std::setlocale(LC_ALL, "") != nullptr and not s_contains((string) std::setlocale(LC_ALL, ""), ";")
        ...
        for (const auto loc_env: array{"LANG", "LC_ALL", "LC_CTYPE"}) {
            if (std::getenv(loc_env) != nullptr and str_to_upper(s_replace((string) std::getenv(loc_env), "-", "")).
                ends_with("UTF8")) {
    ...
    if (cli.preset.has_value()) {
        Config::current_preset = min(static_cast<std::int32_t>(cli.preset.value()),
